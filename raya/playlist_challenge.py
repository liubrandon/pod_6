...
# 'artist' (string)
# 'title' (string)
add_song(my_playlist, {'artist': 'Qveen Herby', 'title': 'Vitamins'})
'''
...
print('Question 5')
add_song(my_playlist, {'artist': 'Yoyo', 'title': 'Cannot Play With My Yoyo'})
add_song(my_playlist, {'artist': 'Tinashe', 'title': 'Bouncing'})
...
mean = monthly_plays[0] + monthly_plays[1] + monthly_plays[2] + monthly_plays[3] + monthly_plays[4]
means = int(mean / 5)
print(f'The average plays per month is {means}.')
...
print('BONUS')
display_playlist(my_playlist)
play_track(my_playlist,track=0)