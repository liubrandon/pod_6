...
def withdraw(account, amount):
    if amount > account['balance']:
        print(f"Your balance is ${account['balance']}, you do not have enough to withdraw this amount")
    else:
        account['balance'] -= amount
...
def deposit_secure(account, amount):
    secure_username = input('Enter username: ')
    secure_password = input('Enter password: ')
    if secure_username == account['username'] and secure_password == account['password']:
        account['balance'] += amount
    else:
        print('Wrong login information, try again')


deposit_secure(my_account, 1000)
print(f"Your new balance is {my_account['balance']}")